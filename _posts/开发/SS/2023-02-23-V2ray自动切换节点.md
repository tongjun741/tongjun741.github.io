    ...
    if serverListLink[i].startswith('ss://'):
      ...
      if len(sys.argv) == 1 or remark.find(filterStr)>-1:
        ...
        if len(sys.argv) == 1 or serverNode['ps'].find(filterStr)>-1:
          ...
          serverListLinkFilter.append(v2Node)

if mode == 'changeNode':
    while True:
        serverListLink = serverListLinkFilter
        setServerNodeId = 0
        if len(serverListLink)>0:
            setServerNodeId = random.randint(0, len(serverListLink)-1)
        print("使用"+serverListLink[setServerNodeId].remark)
...
https://github.com/xbblog95/v2sub