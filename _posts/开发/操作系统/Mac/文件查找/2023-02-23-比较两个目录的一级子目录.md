...
```
find /Users/tongjun/Documents -maxdepth 1 | awk '{gsub(/\/.*\//,"",$1); print}' | sort > /tmp/file1
find /Volumes/MacData/坚果云forMac/Documents -maxdepth 1 | awk '{gsub(/\/.*\//,"",$1); print}' | sort | diff /tmp/file1 - > /tmp/diff.txt
```
...
**强烈谴责大量盗文狗：[波波说运维](https://www.toutiao.com/c/user/3956984439/#mid=1592529616710659)，说不定你们的文章也已被抄袭**

 

曾多次想要在Linux下比较目录a和目录b中**文件列表**的差别，然后对目录a比目录b中多出的文件、少掉的文件分别做处理。但是，在网上搜索了多次也都没找到能直接处理好的工具。

所以想了不少方法，自我感觉都不错，而且网上似乎没有这方面的文章，所以分享出来给大家。如果各位有更好的工具或者方法，盼请留下说明(本文第2部分：`图形化的比较结果`搜集自网上，我也没有在图形化界面下操作的需要，所以没有多做介绍)
...
5. 以上rsync假定了比较的两个目录中只有普通文件和目录，没有软链接、块设备等特殊文件。如果有，请考虑加上对应的选项或者使用-a替代-r，否则结果中将出现skipping non-regular file的提示。但请注意，如果有软链接，且加了对应选项(-l或-a或其他相关选项)，则可能会出现`fileA-->fileB`的输出。
...
![img](/img-post/开发/操作系统/Mac/文件查找/比较两个目录的一级目录.assets/format,png.png)
...
个人建议使用[命令行输出的结果](https://www.cnblogs.com/f-ck-need-u/p/9071033.html#blog1)中的方法方法三和方法四，因为它们都能很好地保留目录前缀。
...
https://blog.csdn.net/l471094842/article/details/100605588